...
class MULTLAModel(nn.Module):
    def __init__(self,hyp_params):
        ...
        Construct a MulT model.
        """
        super(MULTLAModel, self).__init__()
        self.orig_d_l, self.orig_d_a, self.orig_d_v = hyp_params.orig_d_l, hyp_params.orig_d_a, hyp_params.orig_d_v
        self.d_l, self.d_a, self.d_v = 30, 30, 30
        self.vonly = hyp_params.vonly
        self.aonly = hyp_params.aonly
...
class MULTLVModel(nn.Module):
    def __init__(self,hyp_params):
        """
        Construct a MulT model.
        """
        super(MULTLVModel, self).__init__()
        self.orig_d_l, self.orig_d_a, self.orig_d_v = hyp_params.orig_d_l, hyp_params.orig_d_a, hyp_params.orig_d_v
        self.d_l, self.d_a, self.d_v = 30, 30, 30
        self.vonly = hyp_params.vonly
        self.aonly = hyp_params.aonly
        ...
        rnn_hidden = 15
        num_layers = 2
        self.lstm = nn.GRU(hidden_size, rnn_hidden, num_layers,
    ...
    def forward(self, x_l, x_a, x_v):
        ...
        if self.lonly:
            # (V,A) --> L
            h_l_with_as = self.trans_l_with_a(proj_x_l, proj_x_v, proj_x_v)  # Dimension (L, N, d_l)
            #h_l_with_vs = self.trans_l_with_v(proj_x_l, proj_x_v, proj_x_v)  # Dimension (L, N, d_l)
            #h_ls = torch.cat([h_l_with_as, h_l_with_vs], dim=2)
            h_ls = h_l_with_as
            h_ls = self.trans_l_mem(h_ls)
        ...
        if self.vonly:
            # (L,V) --> A
            h_a_with_ls = self.trans_a_with_l(proj_x_v, proj_x_l, proj_x_l)
            #h_a_with_vs = self.trans_a_with_v(proj_x_a, proj_x_v, proj_x_v)
            h_as = h_a_with_ls
            #h_as = torch.cat([h_a_with_ls, h_a_with_vs], dim=2)
...
class MULTVAModel(nn.Module):
    def __init__(self, hyp_params):
        """
        Construct a MulT model.
        """
        super(MULTVAModel, self).__init__()
        self.orig_d_l, self.orig_d_a, self.orig_d_v = hyp_params.orig_d_l, hyp_params.orig_d_a, hyp_params.orig_d_v
        self.d_l, self.d_a, self.d_v = 30, 30, 30
        self.vonly = hyp_params.vonly
        self.aonly = hyp_params.aonly
...
class LSTMModel(nn.Module):
    ...
    def __init__(self, hyp_params):
        ...
        #self.lstm = nn.LSTM(, config.rnn_hidden, config.num_layers,
                            bidirectional=True, batch_first=True, dropout=config.dropout)
        '''
